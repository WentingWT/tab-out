...
function renderBookmarkNode(node) {
  if (node.url) {
    ...
    return `
      <a class="bookmark-item" href="${escapeHtml(node.url)}" title="${escapeHtml(node.title || node.url)}">
  ...
  const children = (node.children || []).filter(child => child.url).slice(0, 24);
  if (children.length === 0) return '';

  return `
    <details class="bookmark-folder">
      <summary>${escapeHtml(node.title || 'Folder')}</summary>
      <div class="bookmark-menu">
        ${children.map(renderBookmarkNode).join('')}
      </div>
    </details>
  `;
}
...
async function renderBookmarkBar() {
  ...
  if (!barEl || typeof chrome === 'undefined' || !chrome.bookmarks) return;
  ...
  try {
    ...
    const bookmarksBar =
      ...
      rootChildren.find(node => node.title === 'Bookmarks bar' || node.title === '书签栏') ||
      rootChildren[0];

    const items = (bookmarksBar?.children || []).slice(0, 48);
    barEl.innerHTML = items.map(renderBookmarkNode).join('');
